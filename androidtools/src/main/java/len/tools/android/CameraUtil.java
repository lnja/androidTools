...
public class CameraUtil {
     ...
     * @return
     */
    public Size getPropSizeForWidth(List<Size> list, int screenWidth) {
        Collections.sort(list, new CameraAscendSizeComparatorForHeight());

        int i = 0;
        for (Size s : list) {
            if ((s.width > screenWidth)) {
                break;
            }
            i++;
        }
        if (Math.abs(list.get(i - 1).width - screenWidth) < Math.abs(list.get(i).width - screenWidth)) {
            i = i - 1;
        }
        return list.get(i);
    }
